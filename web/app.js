...
function renderAuth() {
  ...
  if (state.me) {
    ...
    link.className = 'btn btn--primary btn--google';
    link.append(googleMark(), document.createTextNode(t('signIn')));
    link.href = DEMO ? '#' : `${API}/api/auth/google/start?redirect=${encodeURIComponent(location.href.split('#')[0])}`;
    if (DEMO) {
...
// Google's brand terms require the mark to keep its own colours on a light
// tile -- it may not be recoloured or dropped straight onto a coloured button,
// which ours is. Inline so it costs no request.
const GOOGLE_G =
